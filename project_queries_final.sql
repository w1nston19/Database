...
/*6.Извежда последната версия на поръчките които са със статус изпълнени*/
SELECT * FROM partition_order_history ph
join partition_order p on ph.partition_order_id = p.id
where ph.updated_on = (SELECT MAX(ph2.updated_on)
                       FROM partition_order_history ph2
                       GROUP BY ph2.partition_order_id
                       HAVING ph.partition_order_id = ph2.partition_order_id)
    and ph.is_completed = 1
    order by ph.partition_order_id
